...
from main.utils import get_array_shape_from_lmdb, retrieve_row_from_lmdb
...
class LMDBDataset(Dataset):
    ...
    def __init__(self, dataset_path):
        ...
        self.env = lmdb.open(
            dataset_path, readonly=True, lock=False, readahead=False, meminit=False
        )
    ...
    def __getitem__(self, idx):
        ...
        image = retrieve_row_from_lmdb(
            self.env, "images", self.KEY_TO_TYPE["images"], self.image_shape[1:], idx
        )
        image = torch.tensor(image, dtype=torch.float32)

        label = retrieve_row_from_lmdb(
            self.env, "labels", self.KEY_TO_TYPE["labels"], self.label_shape[1:], idx
        )
