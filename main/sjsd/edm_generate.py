...
import click
import tqdm
from main.sjsd.edm_network import get_cifar10_skewed_edm_config, get_imagenet_edm_config
from main.sjsd.networks import SkewedEDMPrecond
import numpy as np
import torch
import PIL.Image
from third_party.edm.torch_utils import distributed as dist
...
def edm_sampler(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
        ...
        # Euler step.
        denoised = net(x_hat, t_hat, torch.ones_like(t_hat), class_labels).to(
            torch.float64
        )
        ...
        if i < num_steps - 1:
            denoised = net(x_next, t_next, torch.ones_like(t_next), class_labels).to(
                torch.float64
            )
...
def get_cifar10_config():
    ...
    base_config.update(get_cifar10_skewed_edm_config())
    return base_config


def create_generator(checkpoint_path, base_model=None, dataset_name="cifar10"):
    if base_model is None:
        ...
        generator = SkewedEDMPrecond(**base_config)
    ...
    while True:
        try:
            ...
            # Remove fake_unet from the beginning of all keys
            state_dict = {k.replace("fake_unet.", ""): v for k, v in state_dict.items()}
            # Remove key called karras_sigmas
            state_dict = {k: v for k, v in state_dict.items() if k != "karras_sigmas"}
            # Remove all keys starting with cls_pred_branch
            state_dict = {
                k: v
                for k, v in state_dict.items()
                if not k.startswith("cls_pred_branch")
            }
            break
        except:
...
def main(
    ...
    **sampler_kwargs,
    ...
    net = create_generator(network_pkl).to(device)
