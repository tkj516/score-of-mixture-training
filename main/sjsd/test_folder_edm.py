...
def calculate_inception_stats(
    ...
    sigma = torch.ones(
        [feature_dim, feature_dim], dtype=torch.float64, device=accelerator.device
    )
    num_batches = (
    ...
    for i in tqdm(
        range(num_batches // accelerator.num_processes),
        unit="batch",
        disable=not accelerator.is_main_process,
    ):
        images = all_images_tensor[rank_batches[i]]
        features = evaluator(
            images.permute(0, 3, 1, 2).to(accelerator.device), **evaluator_kwargs
        ).to(torch.float64)
        mu += features.sum(0)
    ...
    while True:
        ...
        for checkpoint in new_checkpoints:
            ...
            generator = create_generator(
                os.path.join(checkpoint, "pytorch_model.bin"), base_model=generator, dataset_name=args.dataset_name
            )
