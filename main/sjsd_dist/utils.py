...
def beta_sampler(batch_size: int, alpha: float = 5, beta: float = 2, *args, **kwargs) -> torch.Tensor:
...
def zeroless_uniform_sampler(batch_size: int, *args, **kwargs) -> torch.Tensor:
    # Sample \alpha in the range [0, 1] from a
    # uniform distribution
    return torch.rand([batch_size]) * 0.99 + 0.01
...
def partial_discrete_uniform_sampler(
    batch_size: int, maximum: int, *args, **kwargs
) -> torch.Tensor:
    return torch.randint((maximum - 1) // 2, maximum, [batch_size]).float() / (
        maximum - 1
    )
