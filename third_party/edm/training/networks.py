...
class AttentionOp(torch.autograd.Function):
    ...
    def jvp(ctx, grad_q, grad_k):
        ...
        # Compute the Jacobian-vector product for the forward pass
        grad_q_fp32 = grad_q.to(torch.float32)
        grad_k_fp32 = grad_k.to(torch.float32) / np.sqrt(k.shape[1])
        jvp_w = torch.einsum(
            "ncq,nck->nqk", grad_q_fp32, k.to(torch.float32)
        ) + torch.einsum("ncq,nck->nqk", q.to(torch.float32), grad_k_fp32)
        jvp_w = (
            (jvp_w - (w * jvp_w).sum(dim=2, keepdim=True)).softmax(dim=2).to(q.dtype)
        )
        del ctx.q, ctx.k
        return jvp_w
